...
# Interpretation of Model Comparison:
    # Since in the project instruction the grading is on accuracy, RF is the best model
    # Hwoever, LASSO has good balance across and a better recall score for class 0
...
# Model isn't showing overt errors after cross checking with the dataset

# Note I originally dropped a row when I first started, so the numbers listed may be off, but I
# am going off the class_model_full_print_version.py file which has the correct numbers after I fixed the dropped row issue.