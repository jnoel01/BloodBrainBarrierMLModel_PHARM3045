...
# Interpretation of Model Comparison:
    # I updated my logic for this at the end of the file. I decided to choose RF instead becuase given the context
    # low recall for Class 0 shouldn't matter as much because it's not clinical context, it's discovery context.
...
###################################################################
# STEP 8: TRAIN MODEL (OLD TRAINING, NOT USED FOR FINAL SUBMISSION)
###################################################################
...
########################################################################################################################################################
########################################################################################################################################################
# EDIT!! IMPORTANT: Looking back on the context of RDKit and the descriptors, I actually think that RF is meant to be the best classifier.
# Since it is a bioinformatics tools, we are using is in a discovery pipeline not necesarilly a clinical pipeline, so it's ok
# that the recall for class 0 is lower for RF. The model is intentionally tuned for sensitivity, not specificity and downstream
# assays typically remove false positives... Additionally, the cost of false negatives at discovery outweighs false positives.
# That being said, I'm going to retrain the model on RF instead of LASSO and re-make predictions on the test set!!!
########################################################################################################################################################
########################################################################################################################################################
...
############################################################################
# STEP 8: TRAIN MODEL (FINAL MODEL USED FOR SUBMISSION) RANDOM FOREST MODEL
############################################################################
